...
while True:
	...
	for im in range(1,N): # Каждое устройство 
		if len(machs[im].need_frag_byfreq[machs[im].start_freq])==0:
			continue
		mi = 10000000
		imi = -1
		...
		if imi in dev_byfrag:
			...
			for inm in total_frags[imi]: # на котором такая часть обновления уже скачана.
				...
				if machs[inm].start_freq!=-1 and len(machs[inm].need_frag_byfreq[machs[inm].start_freq])>ma:
					ma = len(machs[inm].need_frag_byfreq[machs[inm].start_freq])
					ima = inm
				# Если и таких устройств оказалось несколько — выбирается устройство с минимальным номером.
				elif machs[inm].start_freq!=-1 and len(machs[inm].need_frag_byfreq[machs[inm].start_freq])==ma and inm<ima:
					ima = inm
			assert ima!=-1
			dev_byfrag[imi] = ima
	...
	for ifrom,li in back_requests.items():
		...
		for ito,inf in li:
			...
			if machs[ifrom].rate_other[ito]>maxrate:
				...
				maxneed = len(machs[ito].need_frag_byfreq[machs[ito].start_freq]) if machs[ito].start_freq!=-1 else 0
			...
			elif machs[ifrom].rate_other[ito]==maxrate and machs[ito].start_freq!=-1 and len(machs[ito].need_frag_byfreq[machs[ito].start_freq])>maxneed: # или у кого поменьше скачано
				selm = ito
				self = inf
				#maxrate = machs[ifrom].rate_other[ito]
				maxneed = len(machs[ito].need_frag_byfreq[machs[ito].start_freq]) if machs[ito].start_freq!=-1 else 0
	...
	for ifrom,(ito,inf) in back_requests.items():
		...
		if len(machs[ito].need_frag_byfreq[machs[ito].start_freq])==0:
			...
			def has(im,inf):
				...
				for k,q in machs[i].need_frag_byfreq.items():
					if inf in q:
						return False
				return True
			print(i,' '.join(['*' if has(i,j) else ' ' for j in range(K)]),machs[i].rate_other,machs[i].need_frag_byfreq)
		print()
